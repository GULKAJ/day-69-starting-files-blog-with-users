...
class Base(DeclarativeBase):
    pass
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("DB_URI", "sqlite:///posts.db").replace("postgresql://", "postgresql+psycopg://")
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if user:
            flash('You are already Signed up with this Email Address. Login instead')
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
        new_comment= Comment(text= comment_form.comment.data, parent_post=requested_post, comment_author=current_user)
